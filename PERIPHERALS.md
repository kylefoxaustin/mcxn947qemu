...
- [x] **Board-to-board (inter-QEMU) transport nodes** — the MCX is a drop-in b2b node on the fleet's link fabric (holobench labs / the i.MX91-93 interconnect), all over a chardev socket, byte-exact:
  ...
  - **CAN** (`tests/mcxn-can-link`) — FlexCAN0 as a CAN-bus client over the fleet `can-host-chardev` backend (`net/can/can_host_chardev.c`, ported from 95); `-object can-bus,id=canbus0 -object can-host-chardev,canbus=canbus0,chardev=<sock>` (the board auto-links `canbus0/1`). A TX message buffer builds a `qemu_can_frame` onto the bus; bus frames land in RX MBs. Both directions byte-exact; internal loopback preserved when no bus.
