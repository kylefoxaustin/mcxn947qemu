...
| `FLEXIO` | 1 | ✅ functional register file + **SPI-master datapath → a real m25p80 NOR**: TX shifter → MOSI / RX shifter ← MISO, CS on a FlexIO output pin; JEDEC-ID + page-program/read-back byte-exact against the flash; mutation-proven; tests/mcxn-flexio-spi-nor. Shifter datapath functional (byte-level, not bit-by-bit); UART/I2S/PWM shifter modes remain a stub |
...
| `SMARTDMA` | 1 | ⚠️ **HONEST-FAULT + register-fidelity** — the EZH datapath is proprietary microcode with **no ISA in the RM** and no reference impl, so it is not modelled: a keyed BOOT runs nothing, moves nothing, raises no completion IRQ, and `CTRL[START]` stays SET (never self-clears over an untouched buffer). The register **protocol** is modelled: `CTRL` is keyed (`0xC0DE` — a keyless command is rejected, not booted), `ARM2EZH` is decoded as `pParam\|mask`, and the boot is decoded to name **which documented op** was requested — the `apiIndex` is recovered from the firmware jump table the SDK installs at SRAMX `0x0400_0000` (`apiIndex 4 = RGB565To888`), exposed via the `last-apiindex`/`last-bootadr` QOM props + an informative honest-fault log. Mutation-proven on the honest-fault gate, the `apiIndex` recovery, and the key check; tests/mcxn-smartdma |
| `SPC` | 1 | ✅ functional |
