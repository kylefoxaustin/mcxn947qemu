...
| `INPUTMUX` | 1 | ✅ functional — register-accurate + real trigger routing: an LPTMR compare (selector 50), a CTIMER0/1/2 match-3 (selectors 5/6/7), or a FlexPWM0/1 SM0 output trigger (selectors 24/25, 32/33) routes to ADCn_TRIG → the ADC's HTEN-gated hardware trigger, so "convert on a timer tick/match/PWM-compare" works (the motor-control synchronous-sampling path). Selectors are PER-DESTINATION where silicon says so: selector 8/9 = CTIMER3/4 M3 for ADC0 but CTIMER3 M2 / CTIMER4 M1 for ADC1 (mutation-proven). The same router drives DACn_TRIG → DAC output-FIFO advance (timer-paced waveform, GCR[TRGSEL]-gated) and CMPn_TRIG → comparator round-robin sampling (RRF flags a deviation, RRCR0[RR_TRG_SEL]-gated). Also gates every eDMA request line (DMAn_REQ_ENABLE). tests/mcxn-adc-hwtrig, mcxn-adc-ctimer-trig, mcxn-adc-ctimer-adc1-trig, mcxn-adc-pwm-trig, mcxn-dac-hwtrig, mcxn-cmp-trig, mcxn-inputmux-gate |
