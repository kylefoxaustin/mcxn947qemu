...
struct MCXNLPUARTState {
    ...
    /* Modem Control @0x40 (the stock EDMA driver uses it) */
    uint32_t mcr;

    /*
     * Extended/timeout registers the SDK driver zeroes during init
     * (0x48..0x6C).
     */
    ...
    /*
     * ⚠ THE RX PATH USED TO BE A SINGLE BYTE (`rx_byte` + `rx_full`) WHILE
     *   THE FIFO REGISTER ADVERTISED AN 8-DEEP RECEIVE FIFO.  The advertisement
     *   was CORRECT -- RM FIFO reset = 0x00C0_0022, RXFIFOSIZE = 010b, and the
     *   RM's own table reads "010b - 8" -- so the model told the truth about
     *   the CHIP and a lie about ITSELF.
     *
     *   ⭐ A CAPABILITY REGISTER IS A CONTRACT.  We promised eight and
     *   delivered one.
     *
     *   And it was INVISIBLE, because RDRF means "RXCOUNT > RXWATER" (RM,
     *   verbatim) and RXWATER resets to 0 -- so with no watermark set,
     *   depth-1 and depth-8 behave identically.  Nothing in 70 suites ever
     *   set a watermark.  A driver that did (enable RXFE, RXWATER=3, wait
     *   for RDRF, read 4) was woken on the FIRST byte and read three stale
     *   ones: SILENT DATA CORRUPTION on the console UART, the
     *   most-exercised block in the tree.
     */
    uint8_t  rx_fifo[MCXN_LPUART_FIFO_DEPTH];
    uint8_t  rx_head;
    uint8_t  rx_count;
    /* STAT[OR]: sticky overrun -- a byte arrived with no room */
    uint32_t stat_or;
    ...
    uint32_t spi_fcr;
    uint32_t spi_tcr;
    /* DMA Enable — the stock EDMA driver sets TDDE/RDDE */
    uint32_t spi_der;
    uint32_t spi_rdr;     /* rx data holding */
    bool     spi_rx_full;
    /* LPSPI chip-select (active low), wired to an SSI device's CS */
    qemu_irq spi_cs;
    /* current CS state, so a burst keeps it low (TCR[CONT]) */
    bool     spi_cs_asserted;
    ...
    /* DMA Enable — the stock EDMA driver sets TDDE/RDDE */
    uint32_t i2c_mder;
    /* rx data holding (1-byte lookahead popped by MRDR) */
    uint32_t i2c_mrdr;
    bool     i2c_rx_full; /* the lookahead byte is valid */
    bool     i2c_busy;    /* asserted between START and STOP */
    /* bytes still to pull from the bus after the lookahead */
    uint32_t i2c_rx_pending;
    /* the real I2C bus — the test attaches an at24c EEPROM to it */
    I2CBus  *i2c_bus;
};
