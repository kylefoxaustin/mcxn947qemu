...
struct MCXNSmartDMAState {
     ...
     * keyed CTRL boot we recover which documented operation was requested —
     * the apiIndex from the firmware jump table at SRAMX, plus the raw entry,
     * the ARM2EZH-carried pParam and its 2-bit mask.  Exposed via QOM so the
     * farm control-plane sees exactly which op a guest asked an un-run engine
     * for.  last_apiindex == 0xFFFFFFFF means "not recovered" (unrecognised
     * firmware or non-standard load address).
     */
