 ...
 * source.  That makes the *honest* model an explicitly-empty one — and,
 * crucially, a LOUD one:
 *
 ...
 *     reached) never sets, so an interrupt/DMA-driven capture blocks rather
 *     than appearing to succeed.
 *   - Reading DATACHn with an empty FIFO sets FIFO_STAT[FIFOUNDn].  A polling
 *     capture therefore gets a detectable underflow instead of a plausible
 *     zero.
 *
 ...
 * exactly like a working one: a keyword-spotter or voice-activity demo records
 * a buffer of perfect digital silence and reports success. A silent zero-stream
 * firmware cannot distinguish from real audio is precisely the top-tier bug
 ...
 * (the single-microphone case), lighting up the watermark / overflow / IRQ
 * paths and — when CTRL_1[DISEL] selects DMA — the MICFIL FIFO request
 * (CMSIS source 18), a FIFO LEVEL (like the SAI), so an operator-fed capture
 * drains over eDMA.
 *
 * Geometry from PARAM (RM §76.7.14, reset 0x0000_0742): NPAIR = 2 (4
 * channels), FIFO_PTRWID = 4 (16-deep per-channel FIFO), 24-bit filter output.
...
struct MCXNPDMState {
    ...
    /* MICFIL0 FIFO request (CMSIS source 18), a FIFO level */
    qemu_irq     dma_req;
    /* current level of dma_req, so we fire only on change */
    bool         dma_lvl;
