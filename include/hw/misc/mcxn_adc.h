...
/*
 * RM ADC chapter: "Supports 16-word depth FIFO with the configurable
 * watermark."  Two FIFOs (0 and 1); TCTRL[FIFO_SEL_A] picks the destination
 * for a conversion.
 */
...
struct MCXNADCState {
    ...
    /*
     * DMA request lines into the eDMA: ADC{n} FIFO A/B = mux sources
     * 21+2n / 22+2n.  Without these the stock LPADC EDMA driver waits forever.
     */
    qemu_irq dma_req[2];
     ...
     *
     * ⚠ THIS USED TO BE `uint32_t fifo_data; bool fifo_valid;` -- A SINGLE
     * SLOT.  A second conversion arriving before the first was drained
     * OVERWROTE it, silently: no flag, no error, a conversion simply
     * ceased to exist.  Real silicon has a 16-word FIFO and raises
     * STAT[FOFn] on overflow, and it drops the NEW result, not the old
     * one (RM: "The newer data is not stored and the FIFO holds the
     * original contents").
     *
     * A depth-1 "FIFO" is not a simplification of a depth-16 one.  It SILENTLY
     * DISABLES THE FEATURE BUILT ON TOP OF IT: FCTRL[FWMARK] is a watermark,
     * and a watermark on a one-deep queue is meaningless -- occupancy can only
     * be 0 or 1, so any firmware asking to be woken at FWMARK >= 1 (i.e. "tell
     * me when 8 samples are ready", the entire point of the register) waited
     * forever.  The model had all the watermark plumbing and nothing to
     * watermark.
     */
