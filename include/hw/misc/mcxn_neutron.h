 ...
 * the compute register map is NOT documented and the SDK ships the engine as
 * binary blobs (libNeutronDriver/libNeutronFirmware).  Disassembly of the M33
 * driver gives the host-visible handshake:
 *
 ...
 *        not NVIC IRQ 97).
 *
 * We cannot run the proprietary Neutron microcode, so this is a FLAG-AT-OPERATOR
 * block: CTRL goes idle synchronously on write so the exec/done spins never hang,
 * but the model NEVER claims to have computed — the inference result is left
 * uncomputed and that truth is exposed to the farm control-plane via QMP
 * (`compute-modelled`=false + `jobs-started`) and a LOG_UNIMP per kick, so a
 * silently-wrong inference is detectable, never hidden.  An operator may opt in
 * to surface the fault to the guest too (`uncomputed-errortrap`) via the
 * non-gating INTR.ERRORTRAP bit + NPU IRQ 97 — never via the completion gate.
 *
