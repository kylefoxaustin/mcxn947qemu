...
/*
 * Trigger source selector numbers.  DERIVED, NOT INVENTED: decoded from NXP's
 * own compiled SDK driver, where kINPUTMUX_Lptmr0ToAdc0Trigger = 0x2800_0032
 * and INPUTMUX_AttachSignal(base, idx, conn) does
 *     *(base + (conn >> 20) + idx * 4) = conn & 0xFFFFF
 * -> "write 50 (0x32) into the register at offset 0x280", which is
 * ADC0_TRIG[0].
 */
#define MCXN_INPUTMUX_SRC_LPTMR0 50
#define MCXN_INPUTMUX_SRC_LPTMR1 51
/*
 * CTIMER{k} match-3 -> ADCn trigger.  kINPUTMUX_Ctimer{k}M3ToAdc0Trigger = 5+k
 * in NXP's driver.  CTIMER0/1/2 use M3 for BOTH ADC0 and ADC1 (collision-free
 * in this shared-selector model); CTIMER3/4 diverge on ADC1 (M2/M1) -- a stated
 * boundary.
 */
...
/*
 * CTIMER3/4 -> ADC selectors 8/9 are PER-DESTINATION: ADC0_TRIG=8/9 name
 * Ctimer3M3 / Ctimer4M3, but ADC1_TRIG=8/9 name Ctimer3M2 / Ctimer4M1 (NXP's
 * connection table).  The shared trig-in source space cannot use the raw
 * selector for both, so the two ADC1-facing match lines get distinct source ids
 * (>65, clear of every selector value) and the router remaps ADC1's selectors
 * 8/9 to them (mcxn_inputmux_trigger).
 */
...
/*
 * eFlexPWM submodule-0 output triggers -> ADCn trigger.
 * kINPUTMUX_Pwm{m}A0Trig{t}ToAdc0 = 24 + 8*m + t in NXP's driver (submodule-0
 * = "A0"; the model runs SM0 only).  These are the motor-control
 * synchronous-sampling path: a VALn compare fires PWM_OUT_TRIGt, which the ADC
 * samples phase-current on, mid-carrier.
 */
...
struct MCXNInputMuxState {
     ...
     * externally-observable behavior in emulation, so storing and reading back
     * the written value is faithful."  THAT JUSTIFICATION IS CIRCULAR: they had
     * no observable behaviour ONLY BECAUSE NOTHING WAS CONNECTED TO THE OTHER
    ...
    /*
     * TRIGGER ROUTING.  ADCn_TRIG[0..3] is a SELECTOR: it names which trigger
     * source drives that ADC trigger input.  One input line per possible
     * selector value, one output line per ADC trigger input; a pulse on source
     * S is forwarded to every destination whose selector currently reads S.
     *
     * Selector 50 = LPTMR0.  That number is not a guess: it was decoded from
     * NXP's own compiled driver, where kINPUTMUX_Lptmr0ToAdc0Trigger =
     * 0x2800_0032 and INPUTMUX_AttachSignal() does  *(base + (conn>>20) +
     * idx*4) = conn & 0xFFFFF, i.e. "write 50 into the register at offset
     * 0x280".
     */
    ...
    /* QDC0/1 position-capture trigger */
    qemu_irq qdc_trig[MCXN_INPUTMUX_NQDC];
