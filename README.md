...
| I3C + EMVSIM — registers/IRQs only, NO DATA PATH YET | B | RETRACTED from tier A on 2026-07-12: mutation-testing showed these move no data at all (I3C MRDATAB, EMVSIM RX_BUF are stubs), and their tests asserted only that an interrupt fired — so a corrupted-data mutation could not fail them. Drivers bind and IRQs are correct; a data path is roadmap. (SAI was in this row and has since earned tier A back.) |
