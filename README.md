...
| USB — device (KHCI + ChipIdea, CDC-ACM) + KHCI HOST enumeration | A | Device: enumerates on stock Linux cdc_acm -> /dev/ttyACM0, bulk byte-exact (tests/mcxn-usb-cdc). Host: the KHCI enumerates a real attached usb-kbd -- device descriptor (VID/PID asserted), SET_ADDRESS, re-read; mutation-proven real-data + bus-reset (tests/mcxn-usb-host) |
