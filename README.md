...
| INPUTMUX — eDMA request gating (DMAn_REQ_ENABLE) | A | Closing a request's gate really BLOCKS the transfer and re-opening it lets it through; an ungated model is MORE PERMISSIVE THAN SILICON, which ships the bug to the board (tests/mcxn-inputmux-gate). TRIGGER ROUTING to the ADC now works BOTH from an LPTMR compare (selector 50, tests/mcxn-adc-hwtrig) AND from a CTIMER match (CTIMER0/1/2 match-3 = selectors 5/6/7, tests/mcxn-adc-ctimer-trig) -- 'convert on a timer match' is the motor-control current-sampling path, and the CTIMER's trigger-EVENT output did not exist before: its match drove an NVIC IRQ and an eDMA request, but the INPUTMUX-routed trigger went NOWHERE, so a periodic-match-paced conversion was impossible (silent, like the LPTMR gap before it). Each is TCTRL[HTEN]-gated (routed-but-unarmed must NOT convert; mutation-proven on both the routing pulse and the HTEN gate). Selectors DERIVED from NXP's compiled driver (kINPUTMUX_Ctimer{k}M3ToAdc0Trigger = 5+k), not guessed. Stated boundaries: CTIMER3/4 route a DIFFERENT match (M2/M1) to ADC1 than to ADC0 (M3), a divergence this shared-selector model does not carry; and FlexPWM->ADC (submodule VAL-compare output trigger) needs mid-period compare timing the reload-paced PWM model does not yet run -- both honest gaps, not faked |
