...
buses (see [Interconnect](#interconnect--board-to-board)). Intended use:
...
compute stubbed but modelable, honestly flagged) · **C** present but proprietary /
out-of-scope compute (no public ISA — Neutron NPU / SmartDMA; honestly flagged to the
guest, never faked) · **N/A** absent on MCXN947 silicon.
...
| Accelerators — proprietary compute (Neutron NPU, SmartDMA) | C | Present but proprietary: the eIQ Neutron NPU and the SmartDMA EZH core run proprietary microcode with no public ISA in the RM, so their compute cannot be modelled honestly (Tier C — present-but-proprietary, cf. a GPU/VPU/NPU). Nothing is faked and nothing hangs: each fails to the guest through its own non-gating channel — the Neutron flags its uncomputed result via INTR[ERRORTRAP] + IRQ 97, and SmartDMA leaves CTRL[START] set because its program never ran and no data was moved (its register protocol is modelled, and a boot is decoded to name the requested op). Host-only flags (QMP/log) are not sufficient — the firmware under test cannot see them (tests/mcxn-neutron, mcxn-smartdma) |
| PowerQuad fixed-point — honest-fault (compute stubbed, modelable) | B | The float matrix/vector + CP0 transcendental path computes (Tier A, above); the fixed-point CP_MTX opcodes (INV/PROD, FFT/FIR) are documented but not modelled yet, so they FAIL to the guest via ERRSTAT[BUSERROR] rather than leaving a stale result buffer — stubbed but doable (Tier B), not proprietary (tests/mcxn-powerquad-coproc) |
...
- **Neutron NPU / SmartDMA run proprietary microcode that is not modelled** — the
  control handshake completes so firmware never hangs, but the uncomputed result is
  faulted **to the guest** through the block's own non-gating channel — the Neutron
  raises `INTR[ERRORTRAP]` + IRQ 97, and SmartDMA leaves `CTRL[START]` set (its
  program never ran and no data moved). It is never silently fabricated, and it is
  never flagged only to the host: a host-only signal (QMP/log) is one the firmware
...
hang. Several depth campaigns are now closed too:

- **Peripheral-triggered DMA is complete** — every eDMA request source is wired
  (SAI, DAC, ADC, the LPFlexcomm serials, SINC, FlexSPI, FlexPWM value-register and
  capture, CTIMER match, SCT event, HsCmp, PinInt, and PDM), so a DMA-shaped stock
  driver no longer hangs on an audio/analog block.
- **The clock tree is fully derived** — the core, bus (`AHBCLKDIV`), timers and
  peripheral clocks follow the configured PLL/divider sources (48 MHz at the FRO_HF
  reset, 150 MHz once firmware brings up PLL0), not hardcoded frequencies.
- **Board-to-board interconnect** over five transports, cross-SoC validated against
  real i.MX 91/93/95 nodes.
- **Upstream-readiness pass** — self-contained (no edits to generic QEMU),
  Kconfig + meson integrated, a MAINTAINERS entry and a board document, and the
  whole tree is checkpatch-clean.

The near-term goal is **upstream submission** to qemu-devel (the machine, board and
`mcxn_*` device models). The honestly-stated open items are: **EMVSIM** needs an
ISO-7816 smartcard model (no card peer exists upstream); the **Neutron NPU** and
**SmartDMA** run proprietary microcode with no public ISA, so — unlike the
Arm-documented Ethos-U65 on the i.MX 9x boards — their compute cannot be modelled
honestly and is faulted to the guest rather than faked; and a few pure input seams
(a PWM-pin capture edge, the external-codec SAI MCLK) have no signal source in
emulation and are exposed as operator/board seams.
...
Maintained by Kyle Fox (see [`MAINTAINERS`](MAINTAINERS)). Cross-validated against
real i.MX silicon and built alongside the emulator fleet (i.MX 91/93/95, RT1180) on
the shared holobench workspace.
