#!/usr/bin/env bash
# MCXN947 FlexSPI execute-in-place (XIP) test: code linked into the FlexSPI0
# AHB NOR window (0x9000_0000) runs in place, no copy to SRAM.
set -euo pipefail
...
echo "$OUT" | grep -q "XIP PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
