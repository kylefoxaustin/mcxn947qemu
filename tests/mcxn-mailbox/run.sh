#!/usr/bin/env bash
# MCXN947 inter-CPU MAILBOX cross-core interrupt test (cpu0<->cpu1 ping-pong).
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
QEMU="${QEMU:-$HERE/../../build/qemu-system-arm}"
CC="${CC:-arm-none-eabi-gcc}"
ELF="$HERE/mailbox.elf"
...
echo "$OUT" | grep -q "MAILBOX PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
