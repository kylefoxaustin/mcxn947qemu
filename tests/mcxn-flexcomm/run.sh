#!/usr/bin/env bash
# MCXN947 LP_FLEXCOMM SPI + I2C function test (LPSPI loopback + LPI2C echo,
# both raising the shared FlexComm NVIC line).
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
QEMU="${QEMU:-$HERE/../../build/qemu-system-arm}"
CC="${CC:-arm-none-eabi-gcc}"
ELF="$HERE/flexcomm.elf"
...
echo "$OUT" | grep -q "FLEXCOMM PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
