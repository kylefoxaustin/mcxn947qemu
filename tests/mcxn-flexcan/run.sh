#!/usr/bin/env bash
# MCXN947 FlexCAN loopback + NVIC interrupt test: asserts a transmitted frame
# loops back into an RX message buffer and raises its interrupt.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
QEMU="${QEMU:-$HERE/../../build/qemu-system-arm}"
CC="${CC:-arm-none-eabi-gcc}"
ELF="$HERE/flexcan.elf"
...
echo "$OUT" | grep -q "FLEXCAN PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
