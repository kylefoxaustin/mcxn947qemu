#!/usr/bin/env bash
# MCXN947 ADC (LPADC) + NVIC interrupt test: asserts a conversion-complete
# interrupt is delivered and a valid result is read from the FIFO.
set -euo pipefail
...
echo "$OUT" | grep -q "ADC PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
