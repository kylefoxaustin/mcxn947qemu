#!/usr/bin/env bash
# MCXN947 dual-core rpmsg-style shared-memory IPC test (real payloads over a
# vring + mailbox doorbell, cpu0<->cpu1).
set -euo pipefail
...
OUT="$(timeout 15 "$QEMU" -M frdm-mcxn947 -display none -monitor none \
        -serial stdio -kernel "$ELF" -no-reboot 2>/dev/null || true)"
echo "--- guest output ---"; echo "$OUT"; echo "--------------------"
echo "$OUT" | grep -q "RPMSG PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
