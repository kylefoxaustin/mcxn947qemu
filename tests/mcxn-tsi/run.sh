#!/usr/bin/env bash
# MCXN947 TSI (touch sense) + NVIC interrupt test: asserts an end-of-scan
# interrupt is delivered and the operator-driven per-channel count is read.
set -euo pipefail
...
echo "$OUT" | grep -q "TSI PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
