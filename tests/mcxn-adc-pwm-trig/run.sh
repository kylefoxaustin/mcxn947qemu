#!/usr/bin/env bash
# FlexPWM0 SM0 VAL0 compare -> PWM_OUT_TRIG0 -> INPUTMUX -> ADC0 hardware trigger ->
# eDMA -> memory.  Motor-control synchronous sampling: the CPU never triggers a
# conversion and never reads RESFIFO; the carrier paces everything.  See main.c.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
QEMU="${QEMU:-$HERE/../../build/qemu-system-arm}"
CC="${CC:-arm-none-eabi-gcc}"
ELF="$HERE/adc-pwm-trig.elf"
...
echo "$OUT" | grep -q "PWM-TRIG PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
