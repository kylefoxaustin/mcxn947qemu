#!/usr/bin/env bash
# CTIMER0 match-3 -> INPUTMUX -> ADC0 hardware trigger -> eDMA -> memory.
# The CPU never triggers a conversion and never reads RESFIFO.  See main.c.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
QEMU="${QEMU:-$HERE/../../build/qemu-system-arm}"
CC="${CC:-arm-none-eabi-gcc}"
ELF="$HERE/adc-ctimer-trig.elf"
...
echo "$OUT" | grep -q "CTIMER-TRIG PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
