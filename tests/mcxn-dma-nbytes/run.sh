#!/usr/bin/env bash
# eDMA with NBYTES that is NOT a multiple of the transfer size: it must be
# REFUSED (CH_ES[NCE], no DONE, destination untouched), not silently truncated.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
QEMU="${QEMU:-$HERE/../../build/qemu-system-arm}"
CC="${CC:-arm-none-eabi-gcc}"
ELF="$HERE/dma-nbytes.elf"
...
echo "$OUT" | grep -q "DMANB PASS" && { echo "PASS"; exit 0; } || { echo "FAIL"; exit 1; }
