...
#
# The chosen suites build WITHOUT CONFIG_USERSPACE so they need no `gperf` host
# tool.  To add userspace/MPU suites (e.g. tests/kernel/common, mem_protect),
# install gperf first: `sudo apt-get install -y gperf`.
set -euo pipefail
...
# Functional kernel suites only.  HW timing-accuracy suites (e.g.
