...
/*
 * ⚠ SPC RESET VALUES -- FROM THE RM, VIA tests/mcxn-reset-values/extract-rm-golden.py.
 *   DERIVED, NEVER INVENTED.  Cross-checked against the CMSIS field masks.
 *
 * This block used to be `memset(regs, 0)` and nothing else -- 13 registers answering
 * zero where the manual says otherwise, and ALL 13 were sitting in the allowlist,
 * excused by a "reason" that was just a restatement of the mismatch.
 *
 * ☠ SPC IS THE SYSTEM POWER CONTROLLER, AND ZERO IS A LEGAL, MEANINGFUL, CATASTROPHIC
 *   VALUE IN IT.  This is 91emulator's dangerous-zero rule landing on the one block
 *   whose entire job is keeping the chip alive:
 *
 *   CNTRL = 0x7 = CORELDO_EN | SYSLDO_EN | DCDC_EN.  ALL THREE REGULATORS ARE ON AT
 *   RESET -- of course they are, the chip is RUNNING.  We answered 0: all three OFF.
 *   And fsl_spc.c read-modify-writes this register:
 *
 *       base->CNTRL |= SPC_CNTRL_CORELDO_EN_MASK;
 *
 *   ⭐ SO A GUEST ENABLING *ONE* REGULATOR READ OUR ZERO, OR'd IN ITS BIT, WROTE BACK
 *      -- AND SILENTLY SWITCHED OFF THE SYS LDO AND THE DC-DC CONVERTER IT NEVER
 *      TOUCHED.  On silicon that is a brownout.  (93emulator's RMW-laundering class,
 *      on the power rails: the wrong reset value is laundered into the guest's OWN
 *      state, and nothing in the model ever reads it back to notice.)
 *
 *   VD_CORE_CFG / VD_SYS_CFG = 0x1 = LVDRE: Low-Voltage-Detect RESET Enable.  We
 *   answered 0 -- BROWNOUT RESET DISABLED.  A guest that RMW'd this to arm the LVD
 *   *interrupt* would silently disarm the RESET that stops the core executing garbage
 *   off a collapsing supply.  The safety mechanism, switched off by reading a lie.
 *
 *   ACTIVE_VDELAY = 0xC8 (200): the core-voltage settle delay.  Zero says the rail
 *   settles INSTANTLY.
 */
