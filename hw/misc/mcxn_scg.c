...
/* [26:25]: 0 ExtClk, 1 Clk48M, 2 Osc32K */
#define PLLCTRL_SOURCE_MASK    0x6000000u
 ...
 * computes the rate from these same registers and programs its timers from
 * the answer.  If our timer ticks at a DIFFERENT rate than the one the
 * driver computed, every delay it derives is wrong BY THAT RATIO -- and
 * nothing anywhere says a word.  That is not a missing feature, it is a
 * SILENT WRONG ANSWER, and it was live: CTIMER ignored its selector and
 * ran at sysclk, so a driver told "you are on FRO_HF, 48 MHz" got a
 * timer running at 150 MHz -- every CTIMER delay 3.1x too short.
 */
/*
 * FRO 48 MHz output (PLL source 1 / Clk48M): available while the FIRC is
 * enabled.
 */
...
/*
 * One PLL's output frequency, from its own CTRL/NDIV/MDIV/PDIV registers.
 * Mirrors the SDK exactly (fsl_clock.c CLOCK_GetPll0OutFreq +
 * findPll0PreDiv/PostDiv/MMult):
 *
 *   Fout = (Fin / N) * M / postdiv
 *     N       = NDIV (min 1), or 1 if BYPASSPREDIV
 *     M       = MDIV (integer; the SSCG fractional path is only used
 *                with SEL_SS_MDIV)
 *     postdiv = 2*PDIV (min 2), or PDIV if BYPASSPOSTDIV2, or 1 if
 *               BYPASSPOSTDIV
 *   Fin per CTRL[SOURCE]: 0 ExtClk(SOSC), 1 Clk48M(FIRC 48M), 2 Osc32K(ROSC).
 *
 * Gated by the PLL being powered/locked (CSR PWREN|CLKEN) -- the same term the
 * read path uses to report the lock bit.  The APLL and SPLL share this layout,
 * so one helper serves both (ctrl/ndiv/mdiv/pdiv/sscg1/csr offsets passed in).
 */
static uint32_t scg_pll_out(MCXNSCGState *s, hwaddr ctrl, hwaddr ndiv,
                            hwaddr mdiv, hwaddr pdiv, hwaddr sscg1, hwaddr csr)
{
    ...
    /*
     * Integer M only.  SSCG fractional multiply (SEL_SS_MDIV) is not
     * modelled; a config that selects it would need the fractional path --
     * flag rather than silently truncate.
     */
...
/*
 * The SCG main clock: RCCR[SCS] selects which source feeds the core/bus
 * tree (the M33 cpuclk and, through SYSCON's AHBCLKDIV, the bus clock).
 * Out of reset RCCR[SCS] = 3 (FRO_HF), so an un-configured core runs at
 * 48 MHz -- exactly as silicon does; BOARD_InitBootClocks brings up PLL0
 * and switches SCS to 5, raising it to 150 MHz.
 */
...
static void mcxn_scg_update_clocks(MCXNSCGState *s)
{
    ...
    /*
     * PLL0/PLL1 outputs — derived from their own dividers (source per
     * CTRL[SOURCE], which for the 150 MHz setup is the 48 MHz FIRC just
     * updated above).
     */
    clock_update_hz(s->apll, scg_pll_out(s, SCG_APLLCTRL, SCG_APLLNDIV,
                                         SCG_APLLMDIV, SCG_APLLPDIV,
                                         SCG_APLLSSCG1, SCG_APLLCSR));
    clock_update_hz(s->spll, scg_pll_out(s, SCG_SPLLCTRL, SCG_SPLLNDIV,
                                         SCG_SPLLMDIV, SCG_SPLLPDIV,
                                         SCG_SPLLSSCG1, SCG_SPLLCSR));
...
static uint64_t mcxn_scg_read(void *opaque, hwaddr off, unsigned size)
{
    ...
    /*
     * ⚠ THESE USED TO BE ONE CASE:  return v | SCG_READY;
     * -- "always valid".
     *
     ...
     *     }
     *
     * -- so a VLD the guest never earned makes the SDK report an EXTERNAL
     * CRYSTAL FREQUENCY FOR AN OSCILLATOR NOBODY TURNED ON.  The RM agrees
     * it is wrong: SOSCCSR/ROSCCSR/APLLCSR/SPLLCSR all reset to 0.  (SIRC
     * and FIRC are genuinely running out of reset and carry their VLD in
     * their RESET VALUE, which is where it belongs -- see mcxn_scg_reset.)
     *
     * So VALID now FOLLOWS ENABLE.  Firmware still never spins -- the clock
     * is "ready" the instant it is enabled, which is the right emulation of
     * a lock time we do not model -- but it is not ready BEFORE THAT, and
     * code that asks "is this oscillator running?" now gets the truth.
     *
     * The enable term per source is taken from what THE DRIVER ACTUALLY SETS
     * before it spins, not from what the field is named:
     *
     *   SOSC:  sets SOSCCM|SOSCEN, waits SOSCVLD            (fsl_clock.c:236)
     *   ROSC:  HAS NO ENABLE BIT AT ALL.  The driver sets ROSCCM and
     *          waits ROSCVLD (fsl_clock.c:355).  A header is a claim
     *          about the silicon; a driver is a claim about what the
     *          silicon must DO.  The driver wins.
    ...
    /*
     * SIRC really is running out of reset -- its VLD (bit 24) is IN ITS
     * RESET VALUE (0x0100_0020), which is where it belongs.
     *
     * ⚠ FIRC IS NOT, AND I FABRICATED ITS VLD FOR A DAY BECAUSE I COULD
     * NOT READ THE RESET VALUE.  The RM's summary row for FIRCCSR says
     * "See section", and the section's bit diagram does not survive
     * pdftotext -- so I left `v | SCG_READY` with a note that I had no
     * authoritative value and would not guess one.
     *
     * That was the right call about the VALUE and the wrong call about
     * the BEHAVIOUR, and the tool told me so only once it started
     * COUNTING the rows the manual DECLINES to answer (91emulator: "a
     * refusal is not a check -- and an UNCOUNTED refusal is not even a
     * refusal").  FIRCCSR was one of 179 rows that were not dropped,
     * not refused, but INVISIBLE.
     *
     ...
     *     // THEN wait
     *
     * It enables FIRC and only then spins on VLD.  A part whose FIRC is
     * already valid before anyone enabled it would make that spin
     * meaningless.
     *
     *     ⭐ A HEADER IS A CLAIM ABOUT THE SILICON; A DRIVER IS A CLAIM
     *        ABOUT WHAT THE SILICON MUST DO.  WHEN THEY DISAGREE -- OR
     *        WHEN THE MANUAL IS SILENT -- THE DRIVER WINS.
     *        (rt1180emulator)
     *
     * So FIRC's VLD now FOLLOWS ITS ENABLE, like SOSC/ROSC/APLL/SPLL.
     * Firmware still never spins: it is valid the instant it is enabled.
     * It is simply not valid BEFORE.  (This is the FIFTH fabricated-ready
     * in this tree, and the last one.)
     */
    case SCG_SIRCCSR:
        return v | SCG_READY;
    case SCG_FIRCCSR:
                           ...
                           uint64_t value, unsigned size)
{
    ...
    /*
     * Re-derive and propagate whenever a register that feeds a source
     * clock, a PLL, or the main-clock select changes.  The source
     * oscillators (SIRC/FIRC) plus the PLL control/divider registers, the
     * PLL power/enable, the external/32k enables, and RCCR[SCS] all move
     * the main clock.
     */
    switch (off) {
    case SCG_SIRCCSR:
 ...
 *
 * ⚠ bit 5 = SIRC_CLK_PERIPH_EN IS SET OUT OF RESET, and resetting this
 * register to ZERO -- which is what memset does -- BREAKS EVERY SDK
 * CLOCK QUERY.
 *
 ...
 *     }
 *
 * With the bit clear that returns 0 Hz, so CLOCK_GetLPFlexCommClkFreq()
 * returns 0, and LPI2C_SlaveInit()/LPUART_Init()/LPSPI_MasterInit() all hit
 ...
 * default; here it is a WRONG one, and it takes out an entire
 * peripheral family.
 *
...
static void mcxn_scg_reset(DeviceState *dev)
{
    ...
    /*
     * RESET VALUES, FROM THE RM's REGISTER MAP.  Every one of these was ZERO,
     * and zero is not a neutral default -- it is a CLAIM, and the SDK
     * computes on it:
     *
     *   RCCR[SCS]      = 3 : the clock source the part is ACTUALLY
     *                        running on out of reset.
     *                        CLOCK_GetCoreSysClkFreq() switches on it; 0 is not
     *                        a source, it is a hole.  (CSR mirrors RCCR.)
     *   xPLLNDIV/MDIV/PDIV = 1 : THE SDK DIVIDES BY THESE.  Zero is a
     *                        divide-by-zero sitting in the model waiting for
     *                        someone to compute a PLL frequency.
     *   xPLLLOCK_CNFG  = 0x4F4C, xPLLSSCG1 = 0x8000_0000, LDOCSR = 8 :
     *                        read-modify-written by the SDK's PLL setup;
     *                        starting from 0 silently drops bits the guest
     *                        never knew it had.
     *   SIRCCSR        = 0x0100_0020 : bit 5 SIRC_CLK_PERIPH_EN is SET
     *                        out of reset, and without it CLOCK_GetFro12MFreq()
     *                        returns 0 Hz and EVERY FlexComm driver asserts
     *                        sourceClock_Hz > 0 and hard-faults.  That one bit
     *                        is what started this audit.
     */
    s->regs[SCG_RCCR >> 2]           = 0x03000000u;   /* SCS = 3 */
    s->regs[SCG_SIRCCSR >> 2]        = SCG_SIRCCSR_RESET;
    /*
     * FIRC is ENABLED + VALID out of reset.  This was the latent inconsistency
     * the read-path comment above already named ("SIRC and FIRC genuinely run
     * out of reset and carry their VLD in their RESET VALUE") but the reset
     * left FIRCCSR at 0.  It is RM-DERIVED, not guessed: RCCR resets to SCS=3
     * (FRO_HF is the boot clock -- confirmed in rm-golden.json, 0x0300_0000),
     * so the FIRC MUST be running at reset, else the core would boot with no
     * clock; and the SDK's own clock_config explicitly sets
     * SCG_FIRCCSR_FIRCEN_CFG=Disabled for its FRO12M profile -- you disable
     * what is on by default.  So FRO_HF = 48 MHz out of reset
     * (FIRCCFG[RANGE]=0), and the un-configured core runs at 48 MHz, exactly
     * as silicon does before BOARD_InitBootClocks.  (FIRCCSR is not in the
     * reset-values golden, so this changes no gated value.)
     */
