...
/* a power-on reset really did just happen */
#define VBAT_STATUSA_POR_DET (1u << 0)

/*
 * The ENABLE bits the READY bits must follow.
 *
 * ⚠ `VBAT_LDOCTLA` USED TO BE 0x200.  THAT IS FROCTLA -- THE 16 kHz FRO's
 *   CONTROL REGISTER.  So STATUSA[LDO_RDY] -- the bit firmware polls to learn
 *   that the RETENTION LDO has come up -- was gated on bit 1 of a DIFFERENT
 *   PERIPHERAL.
 *
 *   Caught by the reset-value golden, which carries the RM's ADDRESSES (via
 *   CMSIS) and not the ones I typed: the manual puts OSCCTLA at 0x100, FROCTLA
 *   at 0x200 and LDOCTLA at 0x300, and the CMSIS VBAT_Type struct agrees to the
 *   byte.  TWO sources I did not author, against one number I did.
 *
 *   ⭐ AN ORACLE YOU DID NOT AUTHOR IS THE ONLY THING THAT CAN CATCH AN
 *      ADDRESS YOU INVENTED -- a test written against the model's own #define
 *      would have agreed with the bug forever.  (Same class as LPI2C at +0x000
 *      where silicon says +0x800.)
 */
...
/*
 * RM reset: STATUSA = 0x81.  Bit 0 is POR_DET -- and that one is TRUE, a
 * power-on reset genuinely just happened.  LDO_RDY (bit 4) and OSC_RDY (bit 5)
 * are NOT set.
 */
...
/*
 * ⚠ VBAT IS WRITE-PROTECTED BY AN INVERSE-PAIR HANDSHAKE, AND WE WERE
 * IGNORING IT.
 *
 * RM 36.x, verbatim:
 *
 ...
 *
 * VBAT is the ALWAYS-ON domain -- the 16 kHz FRO, the 32 kHz crystal, the
 * retention LDO.  The A/B complement pair is a HARDWARE GUARD against a
 * spurious write corrupting the one power domain that survives reset.  The
 * RM's own init sequence:
 *
 *      1. Write 7h to LDOCTLA.
 *      2. Write 0h to LDOCTLB[INVERSE].        <- WITHOUT THIS, NOTHING HAPPENS
 *      3. Wait for STATUSA[LDO_RDY] to become 1.
 *
 * ⭐ WE ACCEPTED THE BARE `LDOCTLA = 7` AND BROUGHT THE LDO UP.  SILICON DOES
 *    NOT.  A model that is TOO FORGIVING does not fail safe -- IT SHIPS THE BUG
 *    TO THE HARDWARE.  Firmware that skipped step 2 worked perfectly here and
 *    would have died on the bench, and the developer would have trusted us over
 *    the board.
 *
 * THE GOLDEN PROVED THE MECHANISM RATHER THAN ME ASSUMING IT: all nine config
 * pairs in the RM's reset column satisfy  B == ~A  within the INVERSE mask,
 * exactly.  The one A register with a NONZERO reset (FROCTLA = 1, the FRO16K
 * runs at power-on) is the one B register that resets to ZERO.  Nine
 * independent confirmations of a mechanism I inferred from a CMSIS field name.
 * (STATUSB is NOT in the table: it is a STATUS register, and the rule says
 * "when CONFIGURING an A register".)
 */
...
/*
 * ⚠ I GUESSED THESE AND THEY WERE WRONG (0x028/0x02C).  They are at
 *   0x7F8/0x7FC.  ONE HOUR after writing "derived, never invented" into a
 *   comment two files away.  The reset-value gate caught it -- an oracle I did
 *   not author, catching an address I made up.  ⭐ THE RULE IS NOT A THING YOU
 *   AGREE WITH.  IT IS A THING YOU DO.
 */
...
/*
 * (A, B, INVERSE mask) -- the mask widths are CMSIS VBAT_*_INVERSE_MASK, not
 * guesses.
 */
static const struct { uint16_t a, b; uint32_t mask; } vbat_pair[] = {
    ...
    /*
     * ⚠ THIS PAIR WAS MISSING, AND THE RESET-TIME ASSERT NEVER NOTICED --
     *   BECAUSE THE ASSERT ITERATES *THIS TABLE*, AND I TYPED THIS TABLE BY
     *   HAND.
     *
     *     ⭐ AN INVARIANT IS ONLY AS COMPLETE AS THE LIST YOU FEED IT.
     *       A guard that checks 9 of 10 pairs gives you the confidence of 10.
     *
     *   The reset-value gate found it -- an oracle I did not author,
     *   catching an omission in a guard I did.  WAKLCKB was reading 0 where the
     *   RM says 1.
     */
...
static const struct { uint16_t off; uint32_t val; } vbat_reset[] = {
    /* STATUSA  (POR_DET: a reset really did just happen) */
    { 0x010, 0x00000081u },
    ...
    /* WAKLCKB  = ~WAKLCKA  (the pair the hand-typed table missed) */
    { 0x7FC, 0x00000001u },
};

/* The guest configured this pair correctly: B holds the inverse of A. */
static bool vbat_pair_ok(MCXNVBATState *s, uint16_t a, uint16_t b,
                         uint32_t mask)
{
...
static bool vbat_locked(MCXNVBATState *s, hwaddr off)
{
    ...
    /*
     * Locked only when the LOCK pair itself is a VALID inverse pair with LOCK
     * set.
     */
...
static uint64_t mcxn_vbat_read(void *opaque, hwaddr off, unsigned size)
{
    ...
    case VBAT_STATUSA: {
         ...
         *                     -- "Oscillator and LDO are always ready in the
         *                        model."
         *
         * "Always ready" is a FABRICATED ASSERTION, and it is the THIRD
         * instance of this exact class in this tree today -- SCG reported four
         * oscillators VALID that nobody turned on, and SYSCON handed out a 1
         * MHz clock nobody selected.  Every time, the mechanism was the same:
         * WE GAVE THE GUEST SOMETHING IT HAD NOT EARNED, and the guest believed
         * us.
         *
         * The RM agrees this was wrong: STATUSA resets to 0x81, and LDO_RDY
         * (bit 4) and OSC_RDY (bit 5) are NOT among those bits.  Bit 0
         * (POR_DET) IS -- and that one is TRUE: a power-on reset really did
         * just happen.
         *
         * So READY now FOLLOWS ENABLE.  Firmware still never spins -- the LDO
         * and the oscillator are ready the instant they are enabled, which is
         * the right emulation of a settling time we do not model -- but they
         * are NOT ready BEFORE that, and code that asks "is the 32 kHz crystal
         * running?" now gets the truth instead of a yes it never asked for.
         */
        uint32_t rdy = 0;

        /*
         * READY follows ENABLE *AND THE INVERSE-PAIR HANDSHAKE*.  Enabling the
         * LDO is a TWO-REGISTER operation on this silicon (LDOCTLA = 7, then
         * LDOCTLB = 0); a guest that writes only the A half has not enabled
         * anything, and the RM's own init sequence polls LDO_RDY precisely to
         * find that out.
         *
         * So firmware that skips the B write now spins here -- exactly as it
         * would on the board.  That is not us hanging the driver; that is the
         * driver discovering, in the emulator, the bug it would otherwise have
         * shipped.  We ALSO say so on the operator's channel, because a spin
         * with no explanation is a bad way to learn it.
         */
        ...
        if ((s->regs[VBAT_LDOCTLA >> 2] & LDOCTLA_LDO_EN) &&
            ...
            qemu_log_mask(LOG_GUEST_ERROR,
                          ...
                          "will not assert\n",
                          s->regs[VBAT_LDOCTLA >> 2],
                          s->regs[VBAT_LDOCTLB >> 2]);
        }
                            ...
                            uint64_t value, unsigned size)
{
    ...
    /*
     * ⭐ ONCE A VBAT BLOCK IS LOCKED, SILICON REFUSES THE WRITE.  We used to
     *   take it.  "More permissive than the hardware" is not the safe
     *   direction -- it means firmware that violates the lock works here and
     *   fails on the board, and the developer trusts us over the silicon.
     *   Fault to the GUEST (the write does not land, so the register reads
     *   back unchanged and the guest can SEE it) and tell the operator why.
     */
    if (vbat_locked(s, off)) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "mcxn_vbat: write 0x%" PRIx64
                      " to LOCKED register @0x%" HWADDR_PRIx
...
static void mcxn_vbat_reset(DeviceState *dev)
{
    ...
    /*
     * THE INVARIANT IS THE POINT, SO ASSERT IT RATHER THAN TRUSTING MY OWN
     * TYPING.
     *
     * Every VBAT config pair must come out of reset with B == ~A within its
     * INVERSE mask -- that is what makes the reset state a VALID configuration
     * rather than a corrupt one.  All nine pairs in the RM's reset column
     * satisfy it exactly; if a future edit to vbat_reset[] breaks one, this
     * trips HERE, at reset, instead of silently handing the guest a pair the
     * silicon would reject.
     *
     * (A reset table is a claim.  This is the claim checking itself.)
     */
    for (i = 0; i < ARRAY_SIZE(vbat_pair); i++) {
        assert(vbat_pair_ok(s, vbat_pair[i].a, vbat_pair[i].b,
                            vbat_pair[i].mask));
    }
    /*
