 ...
 * completes a scan the instant a software trigger is observed: DATA[EOSF]
 * sets and DATA[TSICNT] reads the OPERATOR-SET count for the selected
 * channel — the value a real electrode's capacitance would drive.  The count
 * is NOT invented here; the electrode is the seam and the operator drives it
 * (QOM property).  EOSF and the overrun/out-of-range flags are
 * write-1-to-clear, so the scan-complete handshake finishes.  All other
 * registers are permissively backed.  Offsets/bits from the MCXN947 CMSIS
 * header (TSI_Type).
 *
...
/*
 * A HARDWARE trigger routed in by INPUTMUX from TSI_TRIG (an LPTMR compare):
 * start a scan, the counterpart of the ADC's "convert on a timer tick" for
 * touch sensing -- a low-power loop wakes the TSI periodically off a timer
 * with no CPU involvement.  GENCS[STM] gates it: the routed trigger scans
 * only in HARDWARE-trigger mode (STM=1); in software mode (STM=0) the block
 * is driven by SWTS instead and a stray routed edge must NOT scan.
 */
static void mcxn_tsi_hw_trigger(void *opaque, int n, int level)
{
    ...
    if (!(s->regs[R_GENCS / 4] & GENCS_STM)) {
        /* software-trigger mode: ignore the routed trigger */
        return;
    }
                           ...
                           unsigned size)
{
    ...
    case R_GENCS:
        /*
         * SWTS is a self-clearing software trigger.  GENCS[STM] only SELECTS
         * the trigger source (0 = software / SWTS, 1 = hardware, i.e. the
         * INPUTMUX-routed TSI_TRIG); it does NOT itself start a scan -- a scan
         * in hardware-trigger mode waits for the routed edge
         * (mcxn_tsi_hw_trigger).  The old code scanned on the STM write, which
 ...
 *
 * ☠ SINC[DECIMATION] (bits 20:16) RESETS TO 7.  Ours was ZERO -- A DECIMATION
 *   FACTOR OF ZERO, which is a divide-by-zero in the CIC filter that consumes
 *   it.  That is a dangerous zero in the exact sense this tree has been
 *   chasing all week: legal, meaningful, and catastrophic to anything that
 *   divides by it.
 *
 * The SSC0/1/2 prescalers and thresholds and the GENCS/SHIELD configuration
 * are likewise read-modify-written by the stock touch driver, so our zeros
 * were being laundered into the guest's own configuration (93emulator's
 * class).
 */
...
static void mcxn_tsi_realize(DeviceState *dev, Error **errp)
{
    ...
    /*
     * Hardware trigger routed in by INPUTMUX from TSI_TRIG (LPTMR -> periodic
     * scan).
     */
