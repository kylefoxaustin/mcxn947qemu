...
static void armv7m_realize(DeviceState *dev, Error **errp)
{
    ...
    if (object_property_find(OBJECT(s->cpu), "powerquad")) {
        if (!object_property_set_bool(OBJECT(s->cpu), "powerquad",
                                      s->powerquad, errp)) {
            return;
        }
    }
...
static const Property armv7m_properties[] = {
    ...
    DEFINE_PROP_BOOL("powerquad", ARMv7MState, powerquad, false),
