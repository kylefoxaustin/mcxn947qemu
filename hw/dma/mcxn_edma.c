...
/*
 * CH_SBR[MID] (bits 4:0, CMSIS DMA_CH_SBR_MID_MASK) is the BUS MASTER ID,
 * and its reset value is PER-INSTANCE: the RM gives DMA0 6 and DMA1 7.  We
 * reset it to ZERO -- a different bus master entirely.
 *
 * ⭐ AND CH_SBR IS A REGISTER THE GUEST READ-MODIFY-WRITES.  Linux's fsl-edma
 * does:
 *
 ...
 *
 * so it READ OUR ZERO AND WROTE IT BACK AS ITS OWN CONFIGURATION -- every
 * subsequent transfer issued under the wrong master ID (wrong bus attributes,
 * wrong security domain).  Nothing failed here, because nothing in THIS model
 * reads those bits.  It fails on hardware.
 *
 *     ⭐ A REGISTER THE GUEST READ-MODIFY-WRITES IS THE ONE PLACE A ZERO RESET
 *        VALUE SURVIVES INTO THE GUEST'S OWN STATE.  A zero reset is not the
 *        absence of a claim -- and the RMW registers are where the claim gets
 *        LAUNDERED.
 *                                          (91emulator, same bug, same week)
 *
 * ⚠ AND THE RESET-VALUE GATE COULD NOT SEE THIS.  Its golden stores ONE reset
 * per register name and SHARES IT ACROSS INSTANCES, so the RM giving CH_SBR two
 * values (6 and 7) looked like a CONTRADICTION and the extractor REFUSED all 16
 * of them -- correctly, because a wrong golden makes the checker lie.  But A
 * REFUSAL IS NOT A CHECK: the whole refusal pile turned out to be exactly this
 * register, and nobody was looking at it.  GREP YOUR GOLDEN FOR WHAT THE
 * EXTRACTOR SKIPPED.
 */
...
/*
 * CH_ES — error status (CMSIS DMA_CH_ES_*).  The eDMA VALIDATES THE TCD
 * BEFORE IT MOVES ANYTHING and refuses to run on an error; it does not
 * quietly move a partial minor loop.
 */
...
/* error interrupt enable (CMSIS DMA_CH_CSR_EEI) */
...
/* enable scatter/gather: load the next TCD */
#define TCD_CSR_ESG      (1u << 4)
/* link a channel on major completion */
#define TCD_CSR_MAJORELINK (1u << 5)
#define TCD_CSR_MAJORLINKCH(v)  (((v) >> 8) & 0xF)

/*
 * ATTR: SMOD[15:11] / SSIZE[10:8] / DMOD[7:3] / DSIZE[2:0]
 * (CMSIS DMA_TCD_ATTR_*)
 * SMOD/DMOD define a CIRCULAR BUFFER: the address wraps inside a 2^MOD-aligned
 * block.  These were NOT MODELLED, so the stock wrap_transfer example ran off
 * the end of its source buffer and handed the guest ADJACENT MEMORY --
 * 150000000, 16000000, 32768: the clock constants that happened to live next
 * door.  Not zeros.  PLAUSIBLE NUMBERS.  A silent wrong answer with real data
 * in it.
 */
 ...
 * the link channel.  Masking with 0x7FFF would read LINKCH AS PART OF THE
 * COUNT.
 */
...
/*
 * Advance an address by its offset, honouring the ATTR modulo (circular
 * buffer).  A zero MOD means a plain linear advance.  With MOD = k the address
 * wraps inside its own 2^k-aligned block, which is exactly what makes a ring
 * buffer a ring.
 */
 ...
 * -- IT IS A POINTER TO THE NEXT TCD, which the engine loads on major
 * completion and then runs.  Not modelled, so the stock `scatter_gather` and
 * `ping_pong` examples ran only their FIRST TCD: destination came back
 * "1 2 3 4 0 0 0 0", half the transfer silently missing.
 *
 * The in-memory TCD is 32 bytes, laid out exactly as the channel's TCD
 * registers.
 */
...
static bool edma_minor_loop(MCXNEDMAState *s, int n)
{
    ...
    if (dsize == 0 || nbytes == 0 || citer == 0) {
        /*
         * CITER=0 / NBYTES=0 is a CONFIGURATION ERROR, not a completed
         * transfer.  Setting DONE here told the guest an empty TCD had
         * "finished".
         */
     ...
     *
     * The minor loop below is `for (b = 0; b + step <= nbytes; b += step)`.
     * With NBYTES=6 and a 4-byte transfer size it moved FOUR BYTES AND DROPPED
     * TWO -- no error, no flag, DONE set, INTMAJOR raised, and the guest told
     * that its transfer had completed.  Real eDMA does not do that: a NBYTES
     * that is not a multiple of the transfer size is a CONFIGURATION ERROR
     * (CH_ES[NCE]) and the channel REFUSES TO RUN.
     *
     ...
     *   "ROUND NUMBERS ARE HOW BUGS SURVIVE -- a 2^n sweep sails straight past
     *    the broken values.  I learned the lesson ON the axis that taught it,
     *    and did not carry it ONE COLUMN TO THE RIGHT."
     */
    /*
     * ⚠ THE FULL TCD VALIDATION -- and I nearly shipped a DEAD ERROR CHANNEL
     * while fixing one.  When the NBYTES check went in I DEFINED CH_ES[SAE],
     * [SOE], [DAE] and [DOE] and IMPLEMENTED NONE OF THEM: four error bits that
     * could never be set, in a register a driver reads to find out what went
     * wrong.  Exactly the "defined but never raised" class I had swept for
     * hours earlier.
     *
     * ollama_95_neutron, on his N axis: "I am not predicting it is broken.  I
     * am saying I HAVE NO RIGHT TO SAY IT ISN'T."  My SOFF/SADDR axes had had
     * the same treatment -- every test uses SOFF=4 and an aligned buffer.
     * Round.
     *
     * Real eDMA validates the whole TCD before it moves anything (RM): a
     * misaligned address, an offset that is not a multiple of the transfer
     * size, a NBYTES that is not a multiple of it, or CITER=0, and the channel
     * REFUSES TO RUN.  Silently walking a misaligned buffer 3 bytes at a time
     * produces garbage that no firmware could diagnose.
     */
    err = 0;
    if (saddr % ssize) {
    ...
    if (err) {
        ...
        /*
         * The ERROR interrupt is gated by CH_CSR[EEI] -- NOT by
         * TCD_CSR[INTMAJOR], which is the TRANSFER-COMPLETE signal.  Raising
         * major-completion on an error would tell the guest "done" about a
         * transfer that never ran, which is the very lie this check exists to
         * remove.
         */
        if (c->csr & CH_CSR_EEI) {
            c->intr |= CH_INT_INT;
     ...
     * ENTIRELY.  The loop was `read(saddr, buf, step); write(daddr, buf,
     * step);` with step = SSIZE -- so the WRITE used the SOURCE width.  With
     * SSIZE = 1 and DSIZE = 4 the engine issued FOUR SEPARATE BYTE WRITES where
     * the TCD asked for ONE 32-BIT WRITE.
     *
     ...
     * four byte-pokes at a data register is not one word write -- a FIFO gets
     * four pushes instead of one, and a register with min_access_size = 4
     * rejects them outright.  That is exactly the DMA-to-peripheral path this
     * model now serves.
     *
     * ⭐ AND EVERY TEST IN THIS TREE SET SSIZE == DSIZE.  Each field was swept
     * ALONE and each was individually "correct"; the PAIR was never varied, so
     * the per-axis tests did not merely MISS this -- THEY CERTIFIED IT.
     * ollama_95_neutron, on exactly this shape: "An independent per-axis
     * whitelist will pass a shape that is garbage, with full confidence,
     * because each axis is individually safe.  That is the worst failure a gate
     * can have.  It does not merely MISS the bug -- IT CERTIFIES IT."
     *
     * Correct semantics: the minor loop moves NBYTES.  READS happen in SSIZE
     * chunks (SADDR advances by SOFF each), WRITES in DSIZE chunks (DADDR
     * advances by DOFF each) -- two independent strides.  Both sizes are powers
     * of two, so lcm(ssize, dsize) = max(ssize, dsize), and NBYTES is validated
     * above to be a multiple of both.
     */
     ...
     *
     * ⚠ Per the RM the minor link fires at the end of each minor loop EXCEPT
     * the final one; when CITER reaches 0 the MAJOR loop completes and
     * MAJORELINK fires INSTEAD.  Firing both meant the linked channel ran ONE
     * TIME TOO MANY, and on its extra pass its destination address had already
     * been advanced -- so it wrote PAST THE END of its buffer and corrupted the
     * next object in memory.  In the stock channel_link example that object was
     * the driver's own `g_Transfer_Done` flag: the ISR set it true, the
     * spurious extra transfer stamped it back to false, and main waited forever
     * on a flag that HAD been set.  The interrupt fired, the ISR ran, the CPU
     * took exception 19 and returned cleanly -- and the machine still hung,
     * because the DMA was overwriting the very variable the guest was polling.
     */
    if (citer != 0) {
        if (elink) {
    ...
    if (!(c->tcd_csr & TCD_CSR_ESG)) {
        /*
         * Without scatter/gather DLAST is a signed adjustment.  WITH it,
         * DLAST_SGA is a POINTER to the next TCD and must NOT be added to
         * DADDR.
         */
     ...
     *
     * ⚠ AND THE CHAIN MUST YIELD TO THE GUEST BETWEEN TCDs.  A ping-pong
     * chain is CIRCULAR BY DESIGN (TCD A -> B -> A forever, a continuous
     * double-buffer); the driver's INTMAJOR handler counts iterations and
     * stops it.  Running the chain synchronously never lets that ISR execute,
     * so the machine SPINS -- my first version hung the stock ping_pong example
     * outright, having "fixed" scatter_gather (whose last TCD has ESG=0 and
    ...
    if (c->tcd_csr & TCD_CSR_ESG) {
         ...
         * channel must be re-triggered (by a request or a fresh START) to run
         * it.  My first version RE-ARMED the channel itself, which chained the
         * whole list synchronously.  That "fixed" nothing (scatter_gather
         * already worked once the modulo and the ELINK-aware CITER were right)
         * and it HUNG the stock ping_pong example outright: a ping-pong chain
         * is CIRCULAR by design, and the NXP driver walks it from its own
         * INTMAJOR handler.  My hardware chaining double-processed a list the
         * driver was already managing, and the machine spun.
         *
         * Found by mutation: DISABLING scatter-gather entirely left the test
         * GREEN and made ping_pong PASS.  A feature I added, that was not
         * needed, that broke a working case -- and only breaking it on purpose
         * showed me.
         */
        /* SGE flagged inside on a bad pointer */
        (void)edma_load_next_tcd(s, n);
    }
 ...
 * requests.  My model completed the WHOLE transfer on the first trigger, so
 * the second trigger RAN EVERYTHING AGAIN -- and by then the linked channels'
 * SADDR and DADDR had already advanced past their buffers:
 *
 ...
 * HARD FAULT (CFSR = INVSTATE, HFSR = FORCED).  The "hang" I had been chasing
 * was never a hang: THE GUEST HAD CRASHED, and HardFault_Handler is a
 * while(1).  An exit-1 crash and an exit-1 refusal, one more time.
 *
...
static uint64_t edma_read(void *opaque, hwaddr off, unsigned size)
{
    ...
    if (off < 0x1000) {
        ...
        default:
            if (off >= R_CH_GRPRI &&
                off < R_CH_GRPRI + 4 * MCXN_EDMA_CHANNELS) {
 ...
 *
 * ⚠ THIS MUST RUN IN A BOTTOM HALF, NOT INLINE FROM THE REQUEST.  A
 * peripheral raises its request from inside its OWN MMIO write handler (the
 * SAI asserts when firmware sets TCSR[FRDE]).  If the DMA then wrote straight
 * back into that peripheral, it would be a RE-ENTRANT MMIO access into a
 * device already engaged in I/O, and QEMU's re-entrancy guard DROPS IT:
 *
 *     qemu-system-arm: warning: Blocked re-entrant IO on MemoryRegion: mcxn-sai
 *                      at addr: 0x20
 *
 * The failure is SILENT DATA LOSS and it looks exactly like success: the
 * channel still walks its minor loops, still decrements CITER, still sets
 * DONE and still raises INTMAJOR -- while every byte it "moved" was thrown
 * away and the FIFO stayed empty.  A test that asked "did the transfer
 * complete?" would pass.  Only checking the DATA catches it.  Real DMA is
 * asynchronous anyway; a bottom half is both the correct model and the
 * correct QEMU idiom.
 */
static void mcxn_edma_service_bh(void *opaque)
{
    ...
    do {
        ...
        for (n = 0; n < MCXN_EDMA_CHANNELS; n++) {
            ...
            if (!s->req_enabled[src]) {
                 ...
                 * request is blocked before it ever reaches the engine.
                 * Silicon drops it here; so do we.
                 */
                continue;
            }
            /*
            ...
            progress = true;
            /*
             * An EDGE (timer-match) source fires once and moves exactly one
             * minor loop: auto-ack it here so the drain loop does not re-run it
             * against a level that no FIFO and no write-back will ever lower.
             * A FIFO source (req_edge clear) keeps its level and is
    ...
    /*
     * A pulse (timer-match) is one-shot: the BH gets exactly one chance to
     * service it.  Any edge level still standing here was NOT consumed -- no
     * channel selected that source, or its INPUTMUX gate was closed -- so drop
     * it.  Real match pulses evaporate when nothing is listening; latching one
     * would fire a spurious transfer the moment a channel is later armed on
     * that source.
     */
    {
        int src;
 ...
 * write.  That meant peripheral-triggered DMA, which is how essentially all
 * real audio/ADC/UART/SPI transfer works, DID NOT EXIST: a guest that set ERQ
 * and waited for the FIFO watermark to drive the transfer waited forever.
 *
...
/*
 * An EDGE (pulse) request from a timer-match source (CTIMER, SCT).  The match
 * is an instantaneous event, not a FIFO level: it asks for exactly ONE minor
 * loop and has no level for the drain loop to lower afterwards.  We latch the
 * level so the bottom half sees it, mark the source as edge so the drain loop
 * AUTO-ACKS it after a single minor loop, and kick the BH.  Servicing is
 * deferred to the BH for the same reason as every other source: the request
 * may arrive from the peripheral's own MMIO write, and writing back on that
 * stack is a re-entrant access QEMU drops.
 */
static void mcxn_edma_req_pulse(void *opaque, int src, int level)
{
    ...
    if (!level) {
        /* a pulse: only the asserting edge carries meaning */
        return;
    }
    s->req_edge[src] = true;
...
static void mcxn_edma_reset(DeviceState *dev)
{
    ...
    /*
     * MP_CSR reset = 0x0031_0000 (RM eDMA Management Page Control, both
     * DMA0/DMA1): read-only management-page capability bits [21,20,16] that are
     * not named fields in CMSIS and are not consumed by this model.  It was 0
     * -- a plausible-but-wrong reset the reset-values gate covers now that the
     * golden reaches the eDMA management page.
     */
